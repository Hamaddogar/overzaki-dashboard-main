...
import { Box, Grid, Typography, Paper, Stack, Switch, MenuItem, Divider, Chip, } from '@mui/material';
...
import { createPaymentMothod, deletePaymentMethods, editPaymentMethods, fetchOnePaymentMethods, fetchPaymentMethodsList } from 'src/redux/store/thunks/paymentMethods';
// components
...
export default function OrdersListView() {
  ...
  const handleEditPaymentMethod = (formData: any) => {
    dispatch(editPaymentMethods({ paymentMethodId: editId, data: formData })).then((response: any) => {
      if (response.meta.requestStatus === 'fulfilled') {
        setData(null);
        setOpenPayment({ open: false })
        dispatch(fetchPaymentMethodsList(paymentMethodState.error));
                    ...
                    <Grid item key={indx} xs={12} sm={6} md={4}>
                      <Paper sx={{ borderRadius: '16px' }}>
                        <Grid container item alignItems='center' columnGap="20px" rowGap="10px" sx={{ p: "30px 22px", minHeight: '150px', boxShadow: '0px 6px 20px #00000014', borderRadius: '16px' }}>
                          ...
                          <Grid item xs={12} sx={{ minHeight: '57px', display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                          ...
                          {paymentM?.type === 'Installment Services' && (
                            <Grid item xs={12}>
                              <Typography component='p' variant="subtitle2" sx={{ fontSize: '.8rem', opacity: .8 }} > {paymentM?.description} </Typography>
                            </Grid>
                          )}
                          ...
                          {paymentM?.type === 'Bank Transfer' && (
                            <>
                              <Grid item xs={12}>
                                <Typography component='p' variant="subtitle2" sx={{ fontSize: '.8rem', opacity: .6 }} >IBAN: {paymentM?.bankIBAN} </Typography>
                              </Grid>
                              <Grid item xs='auto'>
                                <Stack direction='row' alignItems='center' spacing='10px'>
                                  ...
                                  <Typography component='p' variant="subtitle2" sx={{ fontSize: '.8rem', opacity: 1 }} > Accepted </Typography>
